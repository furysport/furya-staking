...
const getValidators = ({ client, validatorInfo, delegations }: GetValidatorsParams) => {
    ...
    return client?.alliance.alliancesValidators("migaloo-1")
        .then((data) => {
            const [validators = [], pagination] = validatorInfo || []

            // sum of validator shares
            const totalShares = validators.reduce((acc, v) => acc.plus(v.delegator_shares.toString()), num(0))
            const delegatedValidators = data?.validators as any[]

            const validatorsWithInfo = validators?.map((validator) => {
                ...
                const votingPower = num(100).times(share!).div(totalShares).toFixed(2)
                const commission = num(rate).times(100).toFixed(0)

                return { ...validator,...delegatedValidator, delegated, commission: commission, votingPower }
...
const useValidators = ({address}) : UseValidatorsResult => {
    ...
    const {data: {delegations = []} = {}, isFetched} = useDelegations({address})
