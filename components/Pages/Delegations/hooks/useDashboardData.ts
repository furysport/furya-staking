...
const fetchValidators = async (
  ...
  const allianceValidatorData = await Promise.all(
    validators[0].map(async (v) => ({
      validator: v,
      delegationResponse: (
        await fetchValidatorDelegations(client, address, v.operator_address)
      ).delegations,
    })),
  );
...
const fetchValidatorDelegations = async (
  ...
  const allianceStake = client.alliance.alliancesDelegationByValidator(
    address,
    operatorAddress,
  );
  ...
  const delegations = [
    ...nativeStakeResponse,
    ...allianceStakeResponse.delegations,
  ];
  return { delegations };
...
const fetchDelegations = async (
  ...
  const nativeDelegateCoins = nativeDelegate.map((delegation) => {
    return {
      denom: delegation[0].amount.denom,
      amount: delegation[0].amount.amount,
    };
  });
...
const fetchRewards = async (
  ...
  const allianceRewards = await client.alliance.delegatorRewards(
    address,
    validatorAddress,
    denom,
  );
...
export const useDashboardData = (chainId: string, address: string) => {
    ...
    queryFn: async () => {
      ...
      await Promise.all(
        validators.map(async (v) => {
          const reward = await fetchRewards(
            client,
            chainId,
            address,
  ...
  const isLoading = useMemo(
    () =>
      [validatorQuery, delegationQuery, rewardsQuery].some(
        (query) =>
